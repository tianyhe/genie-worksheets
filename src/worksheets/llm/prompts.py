...
def load_template_file(template_file: str) -> str:
    ...
    """
    raw_template = jinja_environment.loader.get_source(jinja_environment, template_file)[0]
    raw_template = re.sub(jinja2_comment_pattern, "", raw_template)
    raw_template = "\n".join([line.rstrip() for line in raw_template.split("\n")])
    raw_template = re.sub(r"%}\s*", "%}", raw_template)  # remove the white space after {% for ... %} tags etc.
...
def _split_prompt_to_blocks(prompt: str) -> List[Tuple[str, str]]:
    ...
    block_indices = sorted(block_indices)  # sort according to the index they were found at

    # check the prompt format is correct
    assert len([b for b in block_indices if b[1] == "instruction"]) <= 1, (
    ...
    fewshot_start = 1
    assert (num_inputs == num_outputs + 1) or (num_inputs == 0 and num_outputs == 0), (
    ...
    for i in range(len(block_indices)):
        block_content = prompt[
            block_indices_with_end[i][0] + len(block_indices_with_end[i][2]) : block_indices_with_end[i + 1][0]
        ].strip()
...
def _prompt_blocks_to_chat_messages(blocks: List[Tuple[str, str]]) -> Tuple[ChatPromptTemplate, str | None]:
    ...
    for block_type, block in blocks:
        if block_type == "instruction":
            ...
            assert False
        message_prompt_templates.append(block_type.from_template(block, template_format="jinja2"))
...
def load_fewshot_prompt_template(
    ...
    chat_prompt_template = _prompt_blocks_to_chat_messages(template_blocks)
